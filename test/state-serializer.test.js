...
// ===== offsetRecords（コピー&ペーストの貼付位置算出） =====
test('offsetRecords: x/y を平行移動し元レコードは不変（ディープコピー）', () => {
  const src = [{ type: 'charger', id: 'c1', x: 1, y: 2, rotation: 0, label: '', standType: '' }];
  const out = StateSerializer.offsetRecords(src, 0.5, 0.5);
  assert.strictEqual(out[0].x, 1.5);
  assert.strictEqual(out[0].y, 2.5);
  assert.strictEqual(src[0].x, 1, '元レコードを変更しない');
  assert.notStrictEqual(out[0], src[0], '別オブジェクトを返す');
});

test('offsetRecords: dimension の x2/y2、leader の textX/textY も平行移動', () => {
  const dim = StateSerializer.offsetRecords(
    [{ type: 'dimension', id: 'd1', x: 0, y: 0, x2: 2.5, y2: 0, labelOverride: '', color: '#000' }], 1, -1)[0];
  assert.deepStrictEqual([dim.x, dim.y, dim.x2, dim.y2], [1, -1, 3.5, -1]);
  const ld = StateSerializer.offsetRecords(
    [{ type: 'leader', id: 'l1', x: 1, y: 1, textX: 3, textY: 0, lines: ['a'], color: '#000' }], 2, 3)[0];
  assert.deepStrictEqual([ld.x, ld.y, ld.textX, ld.textY], [3, 4, 5, 3]);
});

test('offsetRecords: wiring-route の各頂点を平行移動（routeData をディープコピー）', () => {
  const src = [{ type: 'wiring-route', id: 'w1', routeData: { vertices: [{ x: 0, y: 0 }, { x: 1, y: 2 }], segments: [] } }];
  const out = StateSerializer.offsetRecords(src, 0.5, 0.5);
  assert.deepStrictEqual(out[0].routeData.vertices, [{ x: 0.5, y: 0.5 }, { x: 1.5, y: 2.5 }]);
  assert.strictEqual(src[0].routeData.vertices[0].x, 0, '元頂点を変更しない');
});

test('offsetRecords: 非配列/不正要素でも throw しない', () => {
  assert.deepStrictEqual(StateSerializer.offsetRecords(null, 1, 1), []);
  assert.deepStrictEqual(StateSerializer.offsetRecords(undefined, 1, 1), []);
  const out = StateSerializer.offsetRecords([null, 'x', { type: 'bollard', id: 'b', x: 0, y: 0 }], 1, 1);
